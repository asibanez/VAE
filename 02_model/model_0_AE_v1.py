...
class VAE_model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.fc_latent = nn.Linear(36608, self.h_dim)
    ...
    def forward(self, x):
        ...
        # Encode
        x = self.relu(self.conv_enc_0a(x))              # batch_size x 32 x 218 x 178
        x = self.relu(self.conv_enc_0b(x))              # batch_size x 32 x 218 x 178
        x = self.max_pool(x)                            # batch_size x 32 x 109 x 89

        x = self.relu(self.conv_enc_1a(x))              # batch_size x 64 x 109 x 89
        x = self.relu(self.conv_enc_1b(x))              # batch_size x 64 x 109 x 89
        x = self.max_pool(x)                            # batch_size x 64 x 54 x 44

        x = self.relu(self.conv_enc_2a(x))              # batch_size x 128 x 54 x 44
        x = self.relu(self.conv_enc_2b(x))              # batch_size x 128 x 54 x 44
        x = self.max_pool(x)                            # batch_size x 128 x 27 x 22

        x = self.relu(self.conv_enc_3a(x))              # batch_size x 256 x 27 x 22
        x = self.relu(self.conv_enc_3b(x))              # batch_size x 256 x 27 x 22
        x = self.max_pool(x)                            # batch_size x 256 x 13 x 11 

        x = self.flatten(x)                             # batch_size x (256 x 13 x 11)

        latent = self.relu(self.fc_latent(x))           # batch_size x h_dim

        # Decode
        x = self.relu(self.fc_dec(latent))      		# batch_size x (256 x 13 x 11)
        x = x.reshape(-1, 256, 13, 11)                  # batch_size x 256 x 13 x 11

        x = self.upsample_2(x)                          # batch_size x 256 x 26 x 22
        x = self.relu(self.conv_dec_0a(x))              # batch_size x 256 x 26 x 22 
        x = self.relu(self.conv_dec_0b(x))              # batch_size x 256 x 26 x 22 

        x = self.upsample_2(x)                          # batch_size x 256 x 52 x 44
        x = self.relu(self.conv_dec_1a(x))              # batch_size x 128 x 52 x 44 
        x = self.relu(self.conv_dec_1b(x))              # batch_size x 128 x 52 x 44 

        x = self.upsample_2(x)                          # batch_size x 128 x 104 x 88
        x = self.relu(self.conv_dec_2a(x))              # batch_size x 64 x 104 x 88 
        x = self.relu(self.conv_dec_2b(x))              # batch_size x 64 x 104 x 88 

        x = self.upsample_2(x)                          # batch_size x 64 x 208 x 176
        x = self.relu(self.conv_dec_3a(x))              # batch_size x 32 x 208 x 176 
        x = self.relu(self.conv_dec_3b(x))              # batch_size x 32 x 208 x 176
